...
from .entropy_model_layers import TransformerV2  # V2: modernised decoder
...
class VCTEntropyModel(nn.Module):
    ...
    def __init__(
        ...
        use_v2_encoder: bool = False,
        use_v2_decoder: bool = False,
    ) -> None:
        """
        Temporal Entropy Model
        Args:
            ...
            drop_out_dec: dropout probability in decoder.
                Defaults to 0.0.
            use_v2_encoder: if True, use LongCtxJointEncoder (V2) instead of
                EncoderSection (V1) for encoder_joint.
            use_v2_decoder: if True, use TransformerV2 (modernised) instead of
                Transformer (V1) for decoder.
        """
        super().__init__()
        ...
        # --- encoder_joint: V1 (EncoderSection) vs V2 (LongCtxJointEncoder) ---
        self._use_v2_encoder = use_v2_encoder
        if use_v2_encoder:
            from .modern_blocks import LongCtxJointEncoder
            self.encoder_joint = LongCtxJointEncoder(
                dim=d_model,
                num_heads=num_head,
                num_layers=4,
                expansion=mlp_expansion,
                dropout=drop_out_enc,
                frame_len=window_size_enc ** 2,
                mid_window_frames=16,
            )
        else:
        ...
        # --- decoder: V1 (Transformer) vs V2 (TransformerV2) ---
        self._use_v2_decoder = use_v2_decoder
        if use_v2_decoder:
            self.decoder = TransformerV2(
    ...
    def _get_transformer_output(
        ...
        # V2 encoder has RoPE built in, no external position encoding needed.
        if self._use_v2_encoder:
            encoded_patched = self.encoder_joint(encoded_patched)
        else:
    ...
    def _get_encoded_seqs(
        ...
        if not encoded_seqs:
            raise ValueError("At least one previous latent is required.")

        if len(encoded_seqs) < self.context_len:
            num_pad = self.context_len - len(encoded_seqs)
            encoded_seqs = [encoded_seqs[0]] * num_pad + encoded_seqs
        elif len(encoded_seqs) > self.context_len:
            encoded_seqs = encoded_seqs[-self.context_len :]
