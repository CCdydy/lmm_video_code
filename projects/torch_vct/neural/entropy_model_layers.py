...
# ============================================================================
# V2 additions — modernised TransformerBlock and Transformer wrapper
#
# These are drop-in replacements for the original TransformerBlock / Transformer
# that use RMSNorm, SwiGLU, RoPE, and FlashAttention-2 instead of LayerNorm,
# GELU MLP, LearnedPosition, and WindowMultiHeadAttention.
#
# Usage:
#   from .entropy_model_layers import TransformerBlockV2, TransformerV2
#   decoder = TransformerV2(
#       seq_length=seq_len_dec,
...
from .modern_blocks import RMSNorm, SwiGLU_FFN


class TransformerBlockV2(nn.Module):
    """Modernised TransformerBlock replacing the VCT original.

    Substitutions vs. VCT TransformerBlock:
      LayerNorm                -> RMSNorm
      MLP(GELU)                -> SwiGLU_FFN
      WindowMultiHeadAttention -> FlashAttention-2 + RoPE (handled by FlashAttnBlock)
      LearnedPosition          -> removed (RoPE is applied inside FlashAttnBlock)

    Preserves the 2-subblock structure with self-attn + cross-attn (decoder)
    or self-attn only (encoder), plus the stochastic depth skip.
    """
    ...
    def __init__(
        ...
        super().__init__()
        from .modern_blocks import FlashAttnBlock
        ...
        self.self_attn_block = FlashAttnBlock(
            dim=hidden_dim,
            num_heads=num_heads,
            expansion=mlp_expansion,
            dropout=dropout,
            causal=is_decoder,
            max_seq_len=seq_length if seq_length > 0 else 8192,
        )

        # ---- Block 2: cross-attention (decoder only) ----
        if is_decoder:
            self.norm_2_q = RMSNorm(hidden_dim)
            self.norm_2_kv = RMSNorm(hidden_dim)
            self.q_proj = nn.Linear(hidden_dim, hidden_dim, bias=False)
            self.k_proj = nn.Linear(hidden_dim, hidden_dim, bias=False)
            self.v_proj = nn.Linear(hidden_dim, hidden_dim, bias=False)
            self.out_proj = nn.Linear(hidden_dim, hidden_dim, bias=False)
            self.drop_cross = nn.Dropout(dropout)
            self.ffn_2 = SwiGLU_FFN(hidden_dim, mlp_expansion, dropout)
        else:
            self.norm_2_q = None
    ...
    def forward(
        ...
        Args:
            input: (B, seq_len, dim)
            encoder_output: optional encoder features for cross-attention (decoder).
        ...
        # ---- Block 1: self-attention (FlashAttnBlock handles residual + FFN) ----
        x = self.self_attn_block(input)
        ...
        if self.is_decoder:
            ...
            residual = x
            B, seq_q, D = x.shape
            seq_kv = encoder_output.shape[1]

            q = self.q_proj(self.norm_2_q(x))
            k = self.k_proj(self.norm_2_kv(encoder_output))
            v = self.v_proj(self.norm_2_kv(encoder_output))
            ...
            try:
                ...
                cross_out = flash_attn_func(q_fa, k_fa, v_fa, causal=False)
            except ImportError:
            ...
            cross_out = cross_out.reshape(B, seq_q, D)
            cross_out = self.out_proj(cross_out)
            x = residual + self.drop_cross(cross_out)
...
class TransformerV2(nn.Module):
    """Modernised Transformer stack (replaces VCT `Transformer`).

    Uses TransformerBlockV2 blocks instead of the original TransformerBlock.
    """
    ...
    def forward(
        self, input: Tensor, encoder_output: Optional[Tensor] = None
    ) -> Tensor:
        assert input.dim() == 3, (
            f"Expected (batch_size, seq_length, hidden_dim) got {input.shape}"
        )
